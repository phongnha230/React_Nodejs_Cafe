...
const { validateUserRegistration, validateAdminUserCreation, validateUserLogin, validateId, validatePagination } = require('../middleware/validationMiddleware');
...
router.get('/me', authMW, allow('admin', 'customer', 'staff', 'barista'), auth.me);

// User management routes (admin only)
router.get('/', authMW, allow('admin'), validatePagination, userCtrl.list);
router.post('/', authMW, allow('admin'), validateAdminUserCreation, userCtrl.create);
